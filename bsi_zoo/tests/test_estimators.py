import numpy as np
from bsi_zoo.estimators import reweighted_lasso
...
def _generate_data(n_sensors, n_times, n_sources, nnz):
    ...
    L = rng.randn(n_sensors, n_sources)  # XXX add orientation support
    y = L @ x
...
def test_reweighted_lasso():
    y, L, x, cov = _generate_data(
        n_sensors=50, n_times=1, n_sources=200, nnz=1)
